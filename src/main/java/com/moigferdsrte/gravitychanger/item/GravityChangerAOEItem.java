...
public class GravityChangerAOEItem extends DirectionalGravityItem {
    ...
    public GravityChangerAOEItem(Direction direction, Properties properties) {
    ...
    public @NonNull InteractionResult use(final @NonNull Level level, final @NonNull Player player, final @NonNull InteractionHand hand) {
        ...
        for (LivingEntity target : level.getEntitiesOfClass(LivingEntity.class, player.getBoundingBox().inflate(RADIUS), target -> this.canAffect(player, target))) {
            GravityDirectionUtil.setGravityDirectionWithDownLift(target, this.getDirection());
        }
    ...
    public void appendHoverText(@NonNull ItemStack itemStack, @NonNull TooltipContext context, @NonNull TooltipDisplay display, @NonNull Consumer<Component> builder, @NonNull TooltipFlag tooltipFlag) {
        if (Minecraft.getInstance().hasShiftDown())
            builder.accept(Component.translatable("tooltip.gravity_changer.gravity_changer_aoe").setStyle(
